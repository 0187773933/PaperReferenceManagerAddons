...
// @match        *://webthesis.biblio.polito.it/*
...
var sciHubBaseUrl = 'https://sci-hub.st/';
...
var vipMirrorBaseUrl = '';
var wanfangMirrorBaseUrl = '';
var isCnkiUser = 0;
...
function getSpringerDOI() {
    try {
        // modern Springer Nature layout
        let cite = document.querySelector("p.c-bibliographic-information__citation");

        if (!cite) return false;

        // Example text:
        // "Nature Neuroscience (2024) https://doi.org/10.1038/s41593-024-01234-5"
        let text = cite.innerText || cite.textContent;

        let match = text.match(/\b10\.\d{4,9}\/[-._;()\/:A-Z0-9]+\b/i);
        if (match) return match[0];

        return false;
    } catch(e) {
        console.log("Springer DOI extraction failed", e);
        return false;
    }
}
...
function getIEEEDOI() {
    try {
        ...
        let href = el.href;
        let match = href.match(/\b10\.\d{4,9}\/[-._;()\/:A-Z0-9]+\b/i);

        if (match) return match[0];
        return false;
    } catch(e) {
        console.log("IEEE DOI extraction failed", e);
        return false;
    }
}
...
// IEEE / SPA watcher
/*
function watchForDynamicDOI() {

    if (!/ieeexplore\.ieee\.org/.test(location.href)) return;

    const observer = new MutationObserver(() => {
        let doi = getIEEEDOI();
        if (!doi) return;

        console.log("IEEE DOI detected:", doi);
        floatBox();   // re-run button creation
        observer.disconnect();
    });
...
window.addEventListener('load', watchForDynamicDOI);
*/

function get_most_popular_doi() {
    try{

        if (/springer\.com|nature\.com|link\.springer\.com/i.test(location.href)) {
            let springer = getSpringerDOI();
            if (springer) return springer;
        }

        // ----- IEEE XPLORE -----
        /*
        if (/ieeexplore\.ieee\.org\/abstract\/document/i.test(location.href)) {
            let doi = getIEEEDOI();
            console.log( "ieeexplore ===" , doi );
            if (doi) return doi;
        }
        */

        const doi_regex_1 = /\b(10[.][0-9]{4,}(?:[.][0-9]+)*\/(?:(?!["&'])\S)+)\b/g;
        // const doi_regex_2 = /\b(10[.][0-9]{4,}(?:[.][0-9]+)*\/(?:(?!["&\'<>])\S)+)\b/ig;
        // const doi_regexes = [ doi_regex_1 , doi_regex_2 ];
        const doi_regexes = [ doi_regex_1 ];
        const seen_dois = {};
        function patch_doi( doi ) {
            doi = doi.trim();
            doi = doi.replace(/<\/?[^>]+(>|$)/g, "");
            doi = doi.split("?")[0];
            doi = doi.split(";")[0];
            if ( !seen_dois[ doi ] ) { seen_dois[ doi ] = 1; }
            else { seen_dois[ doi ] += 1; }
        }
       var regex = new RegExp( doi_regex_1 , "g" );
       let test = document.body.innerHTML;
       test.replace( regex , patch_doi );
       let most_popular_doi = Object.keys( seen_dois ).reduce( ( a , b ) => seen_dois[ a ] > seen_dois[ b ] ? a : b );
       return most_popular_doi
    } catch { return false; }
}

var floatBox = function () {
    if ( site.includes( "wikipedia.org" ) ) { return; }
    if ( site.includes( "scholar.google.com" ) ) { return; }
	let x_doi = get_most_popular_doi();
	if ( !x_doi ) { return false }
	let sci_url = `${sciHubBaseUrl}${x_doi}`
	addFloatBox( sci_url , sci_hub_ico );
	let wsu_url = `https://doi-org.ezproxy.libraries.wright.edu/${x_doi}`;
	addWSUFloatBox( wsu_url );
};
...
function addScihubIco(sciHubUrl, documentId, ico) {
	...
	// wsu addon
	if ( sciHubUrl.includes( sciHubBaseUrl ) ) {
		let wsu_link = document.createElement('a');
		wsu_link.setAttribute("target","_blank");
		wsu_link.href = `https://doi-org.ezproxy.libraries.wright.edu/${sciHubUrl.split( sciHubBaseUrl )[ 1 ]}`;
		documentId.parentNode.insertBefore(wsu_link, documentId.nextSibling);
		let wsu_link_img = document.createElement('img');
		wsu_link_img.setAttribute("id", "imgCheck-wsu");
		wsu_link_img.setAttribute("src", wsu_ico);
		wsu_link_img.setAttribute("style", "height: 15px !important;width: 15px !important; margin: 2px !important; display: inline-block;");
		wsu_link.appendChild(wsu_link_img);
	}
}

function isIEEE() {
    return /ieeexplore/i.test(location.hostname)
        || document.querySelector('meta[name="citation_doi"][content^="10.1109"]')
        || document.querySelector('[data-analytics_identifier="document_abstract_doi"]');
}

function addFloatBox(floatLink , ico) {

    let offset = 100;
    if ( isIEEE() ) { offset=150; }

    // reuse existing box if present
    let box = document.getElementById('sciHubButton');
    if (!box) {
    ...
    box.innerHTML =
        `<a target="_blank" href="${floatLink}">
            <img id="imgCheck" src="${ico}" style="height:30px;width:30px;margin:2px;display:inline-block;">
        </a>`;
}

function addWSUFloatBox(floatLink) {

    let offset = 100;
    if ( isIEEE() ) { offset=150; }

    // reuse existing box if present
    let box = document.getElementById('wsuButton');
    if (!box) {
        box = document.createElement('div');
        box.id = 'wsuButton';
        box.style.position = 'fixed';
    ...
    box.innerHTML =
        `<a target="_blank" href="${floatLink}">
            <img id="imgCheck" src="${wsu_ico}" style="height:30px;width:30px;margin:2px;display:inline-block;">
        </a>`;
}


function try_sci_hub_pdf_out() {
    if ( window.location.href.includes( "sci-hub" ) ) {
        var sPDFLink = document.querySelector('#pdf');
        if ( sPDFLink ) {
            if ( sPDFLink.src ) {
                window.location.href = sPDFLink.src;
            }
        }
    }
}

if (window.location.href.includes("epdf")) {
    // Replace 'epdf' with 'pdf' and redirect
    window.location.href = window.location.href.replace("epdf", "pdf");
}

window.addEventListener('load', try_sci_hub_pdf_out);
...
if (location.hostname.includes("pubmed")) {
    return;
}
...
(function injectCSS(){
  ...
  style.textContent=`
  [data-zotero-exists="true"]{
    outline:3px solid #22c55e !important;
    background:rgba(34,197,94,.18) !important;
    border-radius:6px !important;
    padding:4px !important;
  }
  [data-zotero-exists="false"]{
...
function createBadge(exists){

  const badge=document.createElement("span");
  badge.dataset.zoteroBadge="true";

  badge.textContent=exists?"Zotero ✓":"Not in Zotero";
  ...
  badge.style.background=exists?"#22c55e":"#ff9800";
...
function askServer(title){
  return new Promise((resolve,reject)=>{
    http({
      ...
      data:JSON.stringify({ queries:[{ id:"page", title }] }),
...
function findTitleElements(title){
  ...
  const matches=[];
  const nodes=document.querySelectorAll("h1,h2,h3,span,p,div");
...
async function evaluate(){
  ...
  try{
    ...
    document.querySelectorAll("[data-zotero-exists]").forEach(e=>{
      e.removeAttribute("data-zotero-exists");
    });
...
function schedule(){
  ...
  debounce=setTimeout(evaluate,700);
}
