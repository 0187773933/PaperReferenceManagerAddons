...
// @downloadURL  https://github.com/0187773933/PaperReferenceManagerAddons/raw/refs/heads/master/userscripts/doi-button.user.js
...
(function () {
    const DOMAIN_BLACKLIST = [ "docs.google.com" , "pilot.wright.edu" , "auth.wright.edu" , "google.com/maps" , "wikipedia.org" ];
	// ---------------------------------------------------------------------------
...
// Cheap "library changed" token : polled so a page you're already viewing
// recolors in place the moment you save a paper ( no reload ). See pollVersion.
const PRMA_VERSION_API = "http://127.0.0.1:9371/api/version";
...
/* ---------------- LIVE "in Zotero" STATUS ----------------
   The MutationObserver above only paints links it hasn't checked yet, and the
   single-article path is guarded by lastTitle — so once a page is colored it
   never re-checks, and a paper you save AFTER opening the page stays stale
   orange until a reload. Instead we poll the server's cheap /api/version token
   ( bumps whenever the library changes — you saved a paper, the --watch worker
   processed one, etc. ) and, when it moves, re-query + recolor IN PLACE. */

// Re-query every already-colored title and recolor it, then re-run the normal
...
async function repaintExists(){
  ...
  if(marked.length){
    try {
      const r   = await askServerBatch(marked.map(a => (a.innerText || "").trim()));
      const res = (r && r.results) || [];
      marked.forEach((a, i) =>
        a.setAttribute("data-zotero-exists",
          (res[i] && res[i].exists === true) ? "true" : "false"));
    } catch(e){ console.error("[doi-button] repaint error:", e); }
  }
  run();
}
...
function pollVersion(){
  http({
    method: "GET",
    url: PRMA_VERSION_API,
    onload: r => {
      let v;
      try { v = JSON.parse(r.responseText).version; } catch(e){ return; }
      if(v == null) return;
      if(prmaVersion === null){ prmaVersion = v; return; }  // seed only; no repaint
      if(v !== prmaVersion){ prmaVersion = v; repaintExists(); }
    },
